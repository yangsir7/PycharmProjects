...
class crawler:
    ...
    def addlinkref(self, urlFrom, urlTo, linkText):
        lf = self.getentryid('urllist', 'url', urlFrom)
        lt = self.getentryid('urllist', 'url', urlTo)
        self.con.execute('insert into link(fromid,toid) values(%s,%s)' % (lf, lt))
        for i in range(len(linkText)):
            word = linkText[i]
            if word in ignorewords: continue
            wordid = self.getentryid('wordlist', 'word', word)
            self.con.execute('insert into linkwords(wordid,linkid) values(%s,%s)' % (wordid, lt))
